...
| `projectLayout(projectDir)` / `projectLayoutPaths(projectDir)` (+ `…Sync`)                                                                                                                                                                                                                                                       | `'hub' \| 'legacy'` by FliHub's rule (no `hub/` → legacy; `hub/recordings/` → hub; top-level `recordings/` → legacy; else hub), and `{ layout, recordings, transcripts }` — `hub/…` or legacy `recordings/` + `recording-transcripts/`; never merged                                                                               |
